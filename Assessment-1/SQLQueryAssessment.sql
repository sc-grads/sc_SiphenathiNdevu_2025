...
-----------------------------------------22
SELECT ct.ContactTypeID, ct.Name AS ContactTypeName, COUNT(*) AS NoContacts FROM Person.ContactType AS ct
...
CAST(eph.Rate * 40 AS decimal(18,4)) AS salaryinaweek FROM HumanResources.EmployeePayHistory AS eph
...
INNER JOIN HumanResources.Employee AS e ON e.BusinessEntityID = eph.BusinessEntityID
INNER JOIN Person.Person AS p ON p.BusinessEntityID = e.BusinessEntityID
INNER JOIN (SELECT BusinessEntityID, MAX(RateChangeDate) AS LatestChange FROM HumanResources.EmployeePayHistory
...
agg.MinQty AS [Min Quantity], agg.MaxQty AS [Max Quantity] FROM Sales.SalesOrderDetail AS d
...
MAX(d2.OrderQty) AS MaxQty FROM Sales.SalesOrderDetail AS d2
